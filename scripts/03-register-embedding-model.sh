...
MODEL_GROUP_ID=$(curl -s -X POST "$OS_HOST/_plugins/_ml/model_groups/_search" \
...
if [ -n "$MODEL_GROUP_ID" ]; then
  ...
  GROUP_RESPONSE=$(curl -s -X POST "$OS_HOST/_plugins/_ml/model_groups/_register" \
    -H "Content-Type: application/json" \
...
REGISTER_RESPONSE=$(curl -s -X POST "$OS_HOST/_plugins/_ml/models/_register" \
  -H "Content-Type: application/json" \
  -d "{
...
until [ "$STATE" = "COMPLETED" ]; do
  sleep 5
  STATE=$(curl -s "$OS_HOST/_plugins/_ml/tasks/$TASK_ID" | jq -r '.state')
  echo "  state: $STATE"
  if [ "$STATE" = "FAILED" ] || [ "$STATE" = "null" ]; then
    echo "Model registration failed."
    curl -s "$OS_HOST/_plugins/_ml/tasks/$TASK_ID" | jq .
    exit 1
  fi
done

EMBEDDING_MODEL_ID=$(curl -s "$OS_HOST/_plugins/_ml/tasks/$TASK_ID" | jq -r '.model_id')
...
curl -s -X POST "$OS_HOST/_plugins/_ml/models/$EMBEDDING_MODEL_ID/_deploy" | jq .

STATE=""
RETRIES=0
until [ "$STATE" = "DEPLOYED" ]; do
  sleep 5
  STATE=$(curl -s "$OS_HOST/_plugins/_ml/models/$EMBEDDING_MODEL_ID" | jq -r '.model_state')
  ...
  if [ "$STATE" = "DEPLOY_FAILED" ] || [ "$STATE" = "null" ]; then
    ...
    if [ "$RETRIES" -ge 5 ]; then
      ...
      curl -s "$OS_HOST/_plugins/_ml/models/$EMBEDDING_MODEL_ID" | jq .
      exit 1
    fi
  fi
done
