...
**⚠️ Dashboards에서 "Start"(실시간 감지)를 눌러보면 — `INIT` 상태에서 안 넘어갈 수 있음(2026-07-26 실측+공식 문서 확인)**

detector 목록/상세 화면의 "Start" 버튼은 Historical Analysis가 아니라 **실시간(real-time) 감지 job**을 켜는 겁니다(둘은 완전히 다른 기능 — 위 표 참고). 실시간 job은 `detection_interval`(이 레포는 10분)마다 "방금 들어온 새 문서"를 계속 집계해서 판단해야 하는데, **이 레포의 데이터는 `siem-vary`가 배포 시점에 딱 한 번만 만들어서 넣는 정적 스냅샷**이지 계속 흘러 들어오는 라이브 스트림이 아닙니다. 그래서:
- Cold start(공식 문서: 초기 모델을 미리 학습시키는 기능, 최대 10,000개의 과거 데이터 포인트 사용) 자체는 이미 쌓여있는 백필 데이터로 어느 정도 진행될 수 있지만,
- `shingle_size`(기본 8) 만큼 **연속된 구간에 새 데이터가 계속 있어야** 완전히 초기화가 끝나는데, 데이터 적재 시점 이후로는 새로 들어오는 문서가 없으니 그 다음 구간부터는 매번 빈 버킷만 쌓입니다.
- 공식 문서도 "`initialization` 상태로 하루 넘게 머물러 있으면 데이터 누락 구간부터 확인하라"고 안내할 정도로, 이건 흔한 현상입니다.

**결론**: `INIT`에서 안 넘어가거나 넘어가도 그 이후로 결과가 안 나오는 게 이 랩에서는 정상입니다 — 실시간 감지는 "계속 흘러 들어오는 라이브 인덱스"를 위한 기능이고, 이 강의는 Historical Analysis(정적 스냅샷을 소급 분석)만 쓰도록 설계돼 있습니다. 강의 중에는 Dashboards의 "Start"(실시간) 버튼은 누르지 않는 걸 권장 — 눌러도 무해하지만 기대한 결과가 안 나와서 혼란만 생깁니다.
