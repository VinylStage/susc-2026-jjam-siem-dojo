...
| 항목 | 내용 |
|---|---|
| 하는 일 | (등록 전 `max_batch_task_per_node`/`max_running_entities_per_detector_for_historical_analysis` 방어적 재등록 — `CONFIGURATION.md` 참고) → severity/network 이상탐지 Detector 2종 등록 → 각각 Historical Analysis 자동 시작(조회 구간 기본 30일) |
